...
document.getElementById("btn-edit").addEventListener("click",()=>{
  ...
  // sélectionner avatar et interets actuels
  document.querySelectorAll("#edit-avatars .avatar-choice").forEach(a=>{ if(a.src===currentUser.avatar) a.classList.add("selected"); });
  document.querySelectorAll("#edit-interets .interet-btn").forEach(b=>{ if(currentUser.interets.includes(b.textContent)) b.classList.add("selected"); });
});
...
// MATCHS
document.getElementById("btn-match").addEventListener("click",()=>{ showPage("match-page"); displayMatches(); });
...
function displayMatches(){
  ...
  container.innerHTML="";
  if(!users||users.length<2){ container.innerHTML="<p>Pas assez de profils pour matcher</p>"; return; }
  users.forEach(u=>{
...
// MESSAGERIE
document.getElementById("btn-messages").addEventListener("click",()=>{ showPage("messages-page"); loadMessages(); });
...
async function loadData(){
  const res=await fetch(JSONBIN_URL,{ method:"GET", headers:{ "X-Master-Key":MASTER_KEY }});
